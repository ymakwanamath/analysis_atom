...
class Alg_ATOM_cipher:
    ...
    def F(self):
        l=self.l
        output= Xor(l[0], l[5], l[12], l[22] , l[28] , l[37] , l[45] , l[58])
        return(output)
    ...
    def Initialization_Phase(self):
        ...
        for __ in range(rounds):
            ...
            self.n[89]= Xor(value_G, self.l[0], self.key[__%128], z)
    ...
    def Run_Cipher(self, limit):
        ...
        for t in range(limit):
            ...
            self.n[89]=Xor(value_G,self.l[0],self.key[(t)%128])
            for i in range(68):
                self.l[i]=self.l[i+1]
            self.l[68]=value_F
            Z.append(z)
...
def vary_bits(fixed_value, variable_bits):
    n = len(variable_bits)
    for variation in range(0, 2**n):
        value = fixed_value
        for i in range(n):
            bit_position = variable_bits[i]
            if (variation >> i) & 1:
                # Set the bit
...
fixed_value = 0x123456789abcdef0123456789abcdef0  # 128-bit fixed value
variable_bits = [34, 56, 78,13,28]  # bits you want to vary
Equation=[0 for i in range(10)]
for iv in vary_bits(fixed_value, variable_bits):
    cipher=Alg_ATOM_cipher(key, iv,s)
    cipher.Initialization_Phase()
    print("initialization complete")
    Equation=array_Xor(Equation,cipher.Run_Cipher(10))
print("After Cubic attack is")
for i in range(len(Equation)):
    Equation[i]=Equation[i].to_anf()
print(Equation)
'''
cipher=Alg_ATOM_cipher(key, iv, s)
cipher.Initialization_Phase()
...
'''
Equation=[0 for i in range(10)]
for iv in range(0x00000000000000000000000000000007ff):
    cipher=Alg_ATOM_cipher(key, iv,s)
    cipher.Initialization_Phase()
    Equation=array_Xor(Equation,cipher.Run_Cipher(10))
print("After Cubic attack is")
