...
class ATOMCipher:
    ...
    def Run_Cipher(self, limit, fsr, bit_number):
        output_stream=[]
        lfsr_match_keystream=0
        freq_cnt=[0]*128
        freq_key=[0,0]
        freq_nlfr=[0,0]
        freq_z=[0,0]
        ...
        for t in range(limit):
            ...
            self.n[89]=value_G^self.l[0]^self.key[cnt]^self.key[(t)%128]
            ...
            if fsr=='n':
                if z==self.n[bit_number]:
                    lfsr_match_keystream+=1
            if fsr=='l':
                if z==self.l[bit_number]:
                    lfsr_match_keystream+=1
        new=self.binary_array_to_hex(output_stream)
        print(new)
        print("Correlation is")
        print(lfsr_match_keystream/limit)
            #print("For t ="+str(t)+"\n")
            #print(z)
        #print(freq_key)
        #print(freq_nlfr)
    ...
    def Run_Cipher_keycnt(self, limit):
        output_stream=[]
        lfsr_match_keystream=0
        freq_cnt=[0]*128
        freq_key=[0,0]
